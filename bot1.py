...
class BotIlariiaV1(Player):
    ...
    def would_change_card(self) -> bool:
        ...
        if self.opponent[-1] == self.my[-1]:
            self.my.append(self._change_card(self.my[-1], Action.CHANGE))
            return True
        else:
    ...
    def _change_card(self, old_card, action):
        if action.CHANGE:
    ...
    def opponent_changed_card(self):
        self.opponent_said_card(self._change_card(self.opponent[-1], Action.CHANGE))
